...
type InferInputSignalValue<TSignal> =
  TSignal extends InputSignal<infer TValue> ? TValue : never;
...
> implements AfterViewInit
{
  ...
  safeSubmit =
    output<InferOutput<InferInputSignalValue<typeof this.formSchema>>>();
  ...
  private validateOnFormValueChanges() {
    ...
    if (!valueChanges$) {
      throw new Error(
        "[formSchema] Expect NgForm's valueChanges to be present.",
      );
    }
